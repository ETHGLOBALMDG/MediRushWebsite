...
class MedicalRAG:
    ...
    def add_symptom(self, symptom, disease):
        ...
        MedicalRAG.save_statement(f"metta.space().add_atom(E(S(\"symptom\"), S(\"{symptom}\"), S(\"{disease}\")))")
        print("excueted save")
        
    def add_treatment(self, disease, treatment):
        self.metta.space().add_atom(E(S("treatment"), S(disease), ValueAtom(treatment)))
        MedicalRAG.save_statement(f"metta.space().add_atom(E(S(\"treatment\"), S(\"{disease}\"), ValueAtom(\"{treatment}\")))")
        print("excueted save")

    def add_side_effect(self, medicine, side_effect):
        self.metta.space().add_atom(E(S("side_effect"), S(medicine), ValueAtom(side_effect)))
        MedicalRAG.save_statement(f"metta.space().add_atom(E(S(\"side_effect\"), S(\"{medicine}\"), ValueAtom(\"{side_effect}\")))")
        print("excueted save")
